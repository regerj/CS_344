...
char * decrypt(char * msg, char * key)
{
    char decMsg[70000];
    strcpy(decMsg, msg);
    //printf("decMsg before decoding: %s\n", decMsg);

    for(int i = 0; i < strlen(decMsg); i++)
    {
        if(decMsg[i] == 32)
            decMsg[i] = 91;
        if(key[i] == 32)
            key[i] = 91;
        int decChar = (decMsg[i]) - key[i] + 65;
        if(decChar < 65)
            decChar = decChar + 27;
        if(decChar == 91)
            decChar = 32;
        decMsg[i] = (char)decChar;
    }

    //printf("decMsg after decoding: %s\n", decMsg);
    //fflush( stdout );
